...
@click.argument("text")
@click.option("-c", "--color", help="The colors for the gradient (comma seperated hex codes)", default=",".join(random.choice(list(good_gradients.values()))))
@click.option("-f", "--font", help="The font for the figlet (font supported by pyfiglet)", default=random.choice(good_fonts))
...
def cli(text, color, font, all_fonts, pager, direction, justify, width, save_html):
    if all_fonts:
        ...
        for font, color1, color2 in track(
            ...
            figlet_text = figlet_format(text or "Test", font)
            text = Text()
